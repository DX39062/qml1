...
class Net(nn.Module):
    ...
    def forward(self, x):
        ...
        logits = self.fc3(final_x)

        return logits

root_dir = "/home/yangxin202522080302/sci/Hybrid-quantum-classical-algorithm/datasets/"
...
def evaluation(label_pred_list):
    # label_pred_list 里的每个元素应是 [pred_idx, label_idx]，都是整数
    TN = FN = FP = TP = 0
    for p, l in label_pred_list:
        p = int(p); l = int(l)
        if l == 1 and p == 1:
            TP += 1
        elif l == 1 and p == 0:
...
def train(epochs, train_loader, test_loader, model, loss_func_train):
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            logits = model(data_fixed_2d)          # (1, 2)
            target_idx = target.view(-1).long()    # (1,)
            loss = loss_func_train(logits, target_idx)
...
def test(test_loader, mode, model, loss_func_test):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (data, target) in enumerate(test_loader):
            ...
            logits = model(data_fixed_2d)          # (1, 2)
            probs = torch.softmax(logits, dim=1)   # 如果你需要概率就临时算
            pred = probs.argmax(dim=1)             # 预测类别 (1,)
            target_idx = target.view(-1).long()

            pred_target.append([pred.item(), target_idx.item()])

            loss = loss_func_test(logits, target_idx)
            total_loss.append(loss.item())
        ...
        balanced_accuracy = (1/2) * ((TP / (TP+FN+eps))+(TN / (TN+FP+eps)))
...
for no, file in enumerate(filelist):
    ...
    for no_, (train_index, test_index) in enumerate(skf.split(signal_data, signal_label)):
        ...
        inter_list_train_df.to_csv('/home/yangxin202522080302/sci/Hybrid-quantum-classical-algorithm/results/'+str(ROI_info)+')_'+str(no_)+'CV_'+str(ansatz)+'.csv', index=False, header=None)
    
        save_dir = '/home/yangxin202522080302/sci/Hybrid-quantum-classical-algorithm/results/'+str(ROI_info)+'_'+str(no_)+'CV_'+str(ansatz)+'.pth'
        ...
        exp_results_df.to_csv('/home/yangxin202522080302/sci/Hybrid-quantum-classical-algorithm/results/'+str(ROI_info)+'_'+str(no_)+'CV_'+str(ansatz)+'.csv', index=False)
